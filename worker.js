...
/**
 * Get meetings and process at 100 meetings per page
 */
const processMeetings = async (domain, hubId, q) => {
  ...
  while (hasMore) {
      ...
      (
        await (
          await hubspotClient.apiRequest({
            ...
            body: {
              inputs: data.map((meeting) => ({ id: meeting.id })),
            },
          })
        ).json()
      )?.results || []
    const contactIds = new Set(
      contacts.map((a) => a.to.map((c) => c.id)).flat()
    )
    ...
    for (const meeting of data) {
      ...
      const attendees =
        contacts
          .find((c) => c.from.id === meeting.id)
          ?.to.map((t) => contactsResults.find((c) => c.id === t.id)) || []

      for (const attended of attendees) {
        const meetingProperties = {
          ...
          meeting_attende_id: attended.id,
          meeting_attendee_email: attended.properties.email,
        }
        const actionTemplate = {
          includeInAnalytics: 0,
